...
const Quiz = ({ TestName }) => {
  ...
  const handleBeforeUnload = (event) => {
    const confirmationMessage = "Do you want to submit the test?";
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        // Increment the tab switch count when the page becomes hidden
        tabSwitchCount += 1;
  
        if (tabSwitchCount >= 10) {
          // Automatically submit the test after 10 tab switches
          handleSubmitTest();
        }
      }
    };
  ...
  const handleClearSelection = () => {
    dispatch(
      setSolutions({
        questionId: questions[questionNumber - 1]._id,
        optionId: null,
        visitedFlag: true,
      })
    );
      ...
      <div className="bg-gray-900 text-white p-4 min-h-screen">
        ...
        {questions.length == 0 ? (
          ...
          <>
            ...
            className="bg-red-500 text-white px-4 py-2 rounded-md w-full md:w-auto hover:bg-red-600 focus:outline-none focus:ring focus:border-red-300 disabled:cursor-not-allowed"
          >
            Clear
          </button>
